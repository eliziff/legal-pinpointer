...
test('persistent workspace navigates real ranges across documents with stable layout, plus new-tab CanLII launch', {timeout:60000}, async()=>{
  ...
  try{
    ...
    async function newPanel(windowId){
      ...
      await p.evaluate(windowId=>{
        ...
        globalThis.chrome={windows:{getCurrent:async()=>({id:windowId,incognito:false})},tabs:{query:q=>__tabs(q)},
          ...
          runtime:{id:'extension',sendMessage:m=>__send(m),onMessage:{addListener:f=>__messageListeners.push(f)}},sidePanel:{open:v=>__open(v),close:async()=>{}}};
      },windowId);
    ...
    const wait=async s=>p.waitForFunction(s=>document.getElementById('summary').textContent.includes(s),s);
    const fill=async text=>{await p.locator('#query').fill(text);};
    const geometry=()=>p.evaluate(()=>['query','list-viewport','passage-preview','keyboard-help'].map(id=>{const r=document.getElementById(id).getBoundingClientRect();return {id,x:r.x,y:r.y,height:r.height}}));
    const baseGeometry=await geometry();
    await fill('privileg* waiv*');await wait('2 matching paragraphs');
    await p.locator('#query').press('Tab');await wait('1 matching sentences');
    assert.equal(await p.locator('#mode').textContent(),'/s');
    await p.locator('#query').press('Shift+Tab');await wait('3 matching sentences');
    assert.equal(await p.locator('#scope').textContent(),'All tabs');
    assert.match(await p.locator('#detail').textContent(),/3\/4.*1 skipped/);
    await p.locator('#query').press('Shift+Tab');await wait('2 matching sentences');
    assert.equal(await p.locator('#scope').textContent(),'Current tab group');
    await p.locator('#query').press('Shift+Tab');await wait('1 matching sentences');
    await p.locator('#query').press('Shift+Tab');await wait('3 matching sentences');
    ...
    await p.locator('.result-row[data-result="1"]').click();
    ...
    assert.deepEqual(await geometry(),baseGeometry,'Search and preview geometry never move when results change');
    assert.equal(await other.locator('body').innerHTML(),sourceBefore);
    // Actual renderer scroll restoration, with a restarted broker loading issued handles.
    broker=createBroker(api);launcher=createLauncher(api,broker);
    await p.locator('#back').click();await p.waitForFunction(()=>document.getElementById('notice').textContent.includes('Returned'));
    assert.equal(active,1);assert.equal(await other.evaluate(()=>scrollY),0);
    await p.locator('#query').focus();
    await p.keyboard.down('Alt');await p.mouse.wheel(0,130);await p.waitForTimeout(100);
    assert.equal(active,1,'Alt-wheel previews without activating a source tab');
    await p.keyboard.up('Alt');await p.waitForFunction(()=>document.getElementById('notice').textContent.includes('Opened'));
    assert.equal(active,3,'Releasing Alt opens the previewed passage');
    // Clicking an other-window result opens that window's panel in the original click gesture.
    await p.locator('.result-row[data-result="2"]').click();
    await p.waitForFunction(()=>document.getElementById('notice').textContent.includes('Opened'));
    assert.ok(storage['sonar-launch:11']?.handoff);
    ...
    // Alt+Shift+C command route takes no injection path, even from the restricted new tab.
    ...
    await p.waitForFunction(()=>document.getElementById('notice').textContent.includes('CanLII results'));
    ...
    assert.equal(await p.locator('#scope').textContent(),'All tabs');
    // Render stress: 1,000 results, a long preview, stable header, bounded live rows.
    ...
    await api.storage.session.set({'sonar-launch:10':{nonce:'render-test',created:Date.now(),route:'tabs',origin:tabs[0],handoff:{workspace,origin:tabs[0],query:'privileg* waiv*',mode:'s',scope:'all',sequence:Date.now(),result:fake,current:999,scrollTop:131700}}});
    await p.waitForFunction(()=>document.querySelector('#counter').textContent==='1000 / 1000');
    assert.ok(await p.locator('.result-row').count()<=10);
    assert.ok(await p.locator('.result-row[data-result="999"]').count());
    assert.deepEqual(await geometry(),baseGeometry);
    const rows=await p.locator('.result-row').count();
    ...
    assert.equal(await p.locator('.result-row').first().evaluate(el=>el.getBoundingClientRect().height),132);
    assert.equal(await p.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
    await p.locator('#clear').click();await p.waitForFunction(()=>document.getElementById('query').value==='');
    assert.equal(await p.locator('.result-row').count(),0);
    assert.deepEqual(errors,[]);
    console.log(JSON.stringify({renderer:browser.version(),results:1000,mountedRows:rows,layoutStable:true}));
