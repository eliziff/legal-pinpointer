...
await build({...common,entryPoints:[path.join(root,'src/row-worker.mjs')],format:'iife',outfile:path.join(assets,'row-worker.js')});
...
const names=['row-worker.js','rowmodel.onnx','rowmodel-tokenizer.json','rowmodel-tokenizer_config.json','rowmodel.json','decision-worker.js','model.onnx','tokenizer.json','tokenizer_config.json','rl_agent_config.json','ort-wasm-simd-threaded.mjs','ort-wasm-simd-threaded.wasm','pdf.worker.min.mjs','tesseract-worker.js','tesseract-core-simd-lstm.wasm.js','NOTICES.txt','build-info.json'];
...
await emit(before);
for(const name of names.filter(n=>!n.startsWith('rowmodel')||existsSync(path.join(assets,n)))){let part=0;for await(const bytes of createReadStream(path.join(assets,name),{highWaterMark:3*256*1024}))await emit(`<script type="application/octet-stream" data-asset="${name}" data-part="${part++}">${bytes.toString('base64')}</script>\n`);}
