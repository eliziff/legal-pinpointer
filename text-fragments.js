...
(function exposeTextFragments(global) {
  ...
  // As in the legacy structure plane, preformatted line breaks and CanLII's scanned-page lines
  // (LINE_ELEMENTS) end lines.
  function buildCompactIndex(root, structural, mapPoints = true) {
    const chunks = [], runs = [], hiddenCache = new WeakMap(), preformattedCache = new WeakMap();
    ...
    function textNode(node) {
      ...
      const keepLines = structural && value.includes('\n') && isPreformatted(node.parentElement, view, preformattedCache);
      for (const match of value.matchAll(/\s+|\S+/gu)) {
        const text = match[0];
        if (/^\s/u.test(text)) {
          if (keepLines && text.includes('\n')) separator();
          else if (structural) pending ||= { node, offset: match.index };
          else separator();
        } else {
    ...
    while (stack.length) {
      ...
      const block = BLOCK_TAGS.has(node.tagName) || (structural && node.matches(LINE_ELEMENTS));
  ...
  // A text-fragment URL for this range (Tab Sonar passages, Lens), from the same verified builder as
  // the unstructured-passage option: the browser lands on the passage, or this throws.
  function urlForRange(range, root, cleanUrl) {
    if (!range.toString().trim()) throw new Error('No text was selected.');
    const document = range.startContainer.ownerDocument;
    const contextRoot = root && root !== document && root !== document.body ? root : undefined;
    const built = buildPassageLink(range, { contextRoot });
    if (!built) throw new Error('No link can single out this passage on this page.');
    return core.withFragment(cleanUrl, `#:~:${built.directive}`);
  }
