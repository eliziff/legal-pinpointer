(() => {
  ...
  async function collect(){
    ...
    if(document.contentType==='application/pdf')throw new Error('This tab is a PDF viewer, not a readable HTML page.');
  ...
  function resolve(handle,index,start,end,expected){
  ...
  function go(handle,index,start,end,expected){
    const r=resolve(handle,index,start,end,expected);
  ...
  function prepare(handle,index,start,end,expected,mode,options){
    const block=blocks[index];if(!block||block.text!==expected)throw new Error('The source changed. Search again.');
    const range=resolve(handle,index,start,end,block.text.slice(start,end));
    return globalThis.LegalPinpointerLensBridge.formatRange(range,mode,options);
  }
  globalThis.PinpointerLensPage={collect,slice,go,prepare,stop(){stopped=true;}};
})();
